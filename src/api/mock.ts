import type { Vehicle, FenceAlert, FaultAlert, FaultType, BatteryAlert, DrivingAlert, DrivingAlertType, DrivingReport, BatteryMonitorItem, ChargeRecord, DischargeRecord, TripInfo, TrajectoryPoint, TripDetail, TripAlertItem, FenceItem, RepairItem, TenantItem, AssetItem, BizUserItem, BizRoleItem, AuditLog } from '@/types';
...
export function getDashboardStats() {
  ...
  const online = tenantVehicles.filter((v) => v.status === '在线').length;
  ...
  const lowBatteryAlerts = tenantVehicles.filter((v) => v.soc <= 20).length;
...
export function getOnlineVehicles(isCrossTenant: boolean = false): Vehicle[] {
  return getFilteredVehicles(isCrossTenant).filter((v) => v.status === '在线');
}

export function getOfflineVehicles(isCrossTenant: boolean = false): Vehicle[] {
  return getFilteredVehicles(isCrossTenant).filter((v) => v.status === '离线');
}
...
let fenceData: FenceItem[] = [
  { id: 'f1', name: 'Santiago Centro Warehouse', type: 'center', vehicles: ['KLTX51', 'KLTX52', 'KLTX55'], alertType: '出栏预警', status: '生效中', address: 'Av. Libertador Bernardo O\'Higgins 1500, Santiago', time: '2026-01-15 10:00' },
  { id: 'f2', name: 'Valparaiso Port Area', type: 'custom', vehicles: ['KLTX53', 'KLTX56'], alertType: '入栏预警', status: '生效中', address: 'Puerto Valparaiso, Valparaiso', time: '2026-02-20 14:30' },
  { id: 'f3', name: 'Maipu Logistics Park', type: 'custom', vehicles: ['KLTX54', 'KLTX57', 'KLTX58'], alertType: '出栏预警', status: '未生效', address: 'Camino a Melipilla 5000, Maipu', time: '2026-03-10 09:00' },
  { id: 'f4', name: 'Las Condes Depot', type: 'center', vehicles: ['KLTX59'], alertType: '入栏预警', status: '生效中', address: 'Av. Apoquindo 4000, Las Condes', time: '2026-03-22 16:45' },
  { id: 'f5', name: 'Quillota Distribution Center', type: 'custom', vehicles: ['KLTX60', 'KLTX61'], alertType: '出栏预警', status: '生效中', address: 'Ruta 5 Norte Km 120, Quillota', time: '2026-04-01 11:20' },
  { id: 'f6', name: 'Rancagua Service Point', type: 'center', vehicles: ['KLTX62'], alertType: '入栏预警', status: '未生效', address: 'Av. Libertador 800, Rancagua', time: '2026-04-10 08:30' },
];
