...
describe('mock data integrity', () => {
  ...
  it('getBatteryMonitorItems includes vin field', () => {
    ...
    items.forEach(item => {
      ...
      expect(['charging', 'idle', 'preparing', 'discharging', 'fault']).toContain(item.status);
    });
  });
  ...
  it('getBatteryAlerts has valid status and type values', () => {
    ...
    expect(alerts.length).toBeGreaterThan(0);
    const validTypes = ['SOC过低', '电池高温', 'SOC跳变', '充电故障', '温差报警',
  ...
  it('getDrivingAlerts has required fields and valid alert types', () => {
    ...
    expect(alerts.length).toBeGreaterThan(0);
    const validTypes = ['对车一级预警', '对车二级预警', '对车AEB制动', '对人一级预警', '对人二级预警', '对人AEB制动'];
    alerts.forEach(a => {
      expect(a.id).toBeTruthy();
      expect(a.plate).toBeTruthy();
