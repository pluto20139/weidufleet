...
const Monitor: React.FC = () => {
  ...
  const onlineVehicles = useMemo(() => getOnlineVehicles(), []);
  ...
  const locationSection = (
    <div>
      <Card style={{ marginBottom: 16 }} size="small">
        <Space>
          <Input placeholder="搜索企业名称/VIN码/车牌号" maxLength={20} value={plateFilter} onChange={(e) => setPlateFilter(e.target.value)} style={{ width: 240 }} />
          <Button type="primary" icon={<SearchOutlined />}>{t('common.search')}</Button>
        </Space>
      </Card>
      <Row gutter={16}>
        <Col flex="280px">
          <Card size="small" title="智利物流集团" style={{ marginBottom: 8 }}>
            <Tree
              checkable
              defaultExpandAll
                ...
                <CircleMarker key={v.vin} center={[v.lat, v.lng]} radius={7} pathOptions={{ color: '#52c41a', fillColor: '#52c41a', fillOpacity: 0.7 }}>
                  <Popup>
                    <div style={{ minWidth: 200 }}>
                      ...
                      <div><strong>企业:</strong> 智利物流集团</div>
  ...
  const trajectorySection = (
    <div ref={trajRef}>
      ...
      <div style={{ display: 'flex', gap: 16, marginBottom: 16 }}>
        ...
        <Card
          size="small"
          title={<span style={{ fontSize: 13, fontWeight: 600 }}><CarOutlined style={{ marginRight: 6, color: '#4f46e5' }} />车辆列表</span>}
            ...
            {trajectoryPositions.length > 0 && (
              <>
                <Marker position={trajectoryPositions[0]!}>
                  <LTooltip permanent><span style={{ color: '#22c55e', fontWeight: 600 }}>起</span></LTooltip>
                </Marker>
                <Marker position={trajectoryPositions[trajectoryPositions.length - 1]!}>
                  <LTooltip permanent><span style={{ color: '#ef4444', fontWeight: 600 }}>终</span></LTooltip>
                </Marker>
              </>
            )}
          </MapContainer>
      ...
      <Card style={{ borderRadius: 8, border: '1px solid #e5e7eb', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }} size="small"
        title={<span style={{ fontWeight: 600, fontSize: 14, color: '#1e293b' }}>行程记录</span>}>
        <Table dataSource={trips}
          columns={[
            { title: '车牌号', dataIndex: 'plate', key: 'plate', width: 100, render: (v: string) => maskPlate(v) },
            {
              ...
              render: (v: string) => v ? (
                <Tooltip title={truncateLocation(v)} placement="topLeft">
                  <span>{truncateLocation(v)}</span>
                </Tooltip>
              ) : '—'
            },
            {
