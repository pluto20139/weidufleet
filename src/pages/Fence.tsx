...
const genFences = (): ExtFenceItem[] => {
  const base = [
    { name: '圣地亚哥中心仓库', type: 'center' as const, vehicles: ['KLTX51', 'KLTX52'], alertType: '出栏预警' as const, address: '智利圣地亚哥解放者大道1500号', center: [-33.45, -70.65] as [number, number], radius: 3000 },
    { name: '瓦尔帕莱索港区', type: 'custom' as const, vehicles: ['KLTX53'], alertType: '入栏预警' as const, address: '— —', polygon: [[-33.04, -71.62], [-33.05, -71.60], [-33.03, -71.61]] as [number, number][] },
    { name: '马伊普物流园', type: 'center' as const, vehicles: ['KLTX54', 'KLTX55'], alertType: '出栏预警' as const, address: '智利圣地亚哥马伊普区工业大道200号', center: [-33.48, -70.72] as [number, number], radius: 2000 },
    { name: '拉斯孔德斯商务区', type: 'center' as const, vehicles: ['KLTX56'], alertType: '入栏预警' as const, address: '智利圣地亚哥拉斯孔德斯区', center: [-33.41, -70.57] as [number, number], radius: 1500 },
    { name: '兰卡瓜维修站', type: 'custom' as const, vehicles: ['KLTX57', 'KLTX58'], alertType: '出栏预警' as const, address: '— —', polygon: [[-34.07, -70.72], [-34.08, -70.70], [-34.06, -70.71]] as [number, number][] },
  ];
...
const Fence: React.FC = () => {
  ...
  const [fences, setFences] = useState<ExtFenceItem[]>([]);

  useEffect(() => {
    const tenantPlates = new Set(allVehicles.map((v) => v.plate));
    const filteredFences = initialFences.filter(
      (f) => f.vehicles.length === 0 || f.vehicles.some((p) => tenantPlates.has(p))
    );
    setFences(filteredFences);
  }, [allVehicles]);
  ...
  const handleOpenEdit = (fence?: ExtFenceItem, viewOnly = false) => {
    ...
    if (fence) {
      ...
      form.resetFields();
      form.setFieldsValue({ type: 'center', alertType: '出栏预警', radius: 1000 });
      setFenceType('center');
            ...
            <Card style={{ width: '100%' }}>
              <Form form={form} layout="vertical" disabled={isViewing}>
                ...
                <Form.Item name="alertType" label={t('fence.alert_type')} rules={[{ required: true }]}>
                  <Radio.Group>
                    <Radio value="出栏预警">{t('fence.exit')}</Radio>
                    <Radio value="入栏预警">{t('fence.entry')}</Radio>
                  </Radio.Group>
                </Form.Item>
                {fenceType === 'center' && (
                  <>
