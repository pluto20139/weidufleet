...
const Battery: React.FC = () => {
  ...
  const monitorItems = useMemo(() => getBatteryMonitorItems(), [tenant]);
  const chargeRecords = useMemo(() => getChargeRecords(), [tenant]);
  const dischargeData = useMemo(() => getDischargeRecords(), [tenant]);
  const dailyConsumption = useMemo(() => getDailyConsumption(30), [tenant]);
  ...
  const filteredMonitorItems = useMemo(() => {
    return monitorItems.filter((item) => {
      if (monPlate && !item.plate.toLowerCase().includes(monPlate.toLowerCase())) return false;
      return true;
    });
  }, [monitorItems, monPlate]);

  const filteredChargeRecords = useMemo(() => {
    return chargeRecords.filter((r) => {
      if (chargePlate && !r.plate.toLowerCase().includes(chargePlate.toLowerCase())) return false;
      if (chargeDateRange && chargeDateRange[0] && chargeDateRange[1]) {
        const t = new Date(r.time).getTime();
        const start = chargeDateRange[0].valueOf();
        const end = chargeDateRange[1].valueOf();
        if (t < start || t > end) return false;
      }
      return true;
    });
  }, [chargeRecords, chargePlate, chargeDateRange]);

  const filteredDischargeRecords = useMemo(() => {
    return dischargeData.filter((r) => {
      if (dischargePlate && !r.plate.toLowerCase().includes(dischargePlate.toLowerCase())) return false;
      if (dischargeDateRange && dischargeDateRange[0] && dischargeDateRange[1]) {
        const t = new Date(r.time).getTime();
        const start = dischargeDateRange[0].valueOf();
        const end = dischargeDateRange[1].valueOf();
        if (t < start || t > end) return false;
      }
      return true;
    });
  }, [dischargeData, dischargePlate, dischargeDateRange]);
  ...
  const monitorColumns = [
    ...
    {
      title: t('battery.daily_consumption', '日均电耗'),
      dataIndex: 'dailyConsumption',
      key: 'dailyConsumption',
      render: (v: number) => `${v || 0} kWh/100km`,
    },
    { title: t('battery.charges', '累计充电次数'), dataIndex: 'charges', key: 'charges' },
    {
      title: t('battery.status', '充电状态'),
      dataIndex: 'status',
      key: 'status',
      render: (status: string) => {
        const stateMap: Record<string, { text: string; color: string }> = {
          idle: { text: t('battery.state.idle', '未充电'), color: 'default' },
          preparing: { text: t('battery.state.preparing', '准备充电'), color: 'processing' },
          charging: { text: t('battery.state.charging', '充电中'), color: 'success' },
          discharging: { text: t('battery.state.discharging', '对方放电'), color: 'warning' },
          fault: { text: t('battery.state.fault', '故障&异常'), color: 'error' },
        };
        const info = stateMap[status] || { text: status, color: 'default' };
  ...
  const monitorTab = (
    <div>
      ...
      <Table
        dataSource={filteredMonitorItems}
      ...
      >
        {detailItem && (() => {
          const vehicle = vehicles.find(v => v.vin === detailItem.vin);
          return (
          <div>
            <Descriptions column={3} bordered size="small" style={{ marginBottom: 16 }}>
              <Descriptions.Item label="VIN">{maskVin(detailItem.vin)}</Descriptions.Item>
              <Descriptions.Item label={t('veh.plate')}>{maskPlate(detailItem.plate)}</Descriptions.Item>
            ...
            <Row gutter={16} style={{ marginBottom: 16 }}>
              ...
              <Col span={6}>
                <Card size="small">
                  <Statistic title={t('battery.range')} value={detailItem.range} suffix="km" />
                </Card>
              </Col>
            </Row>
            <Row gutter={16} style={{ marginBottom: 16 }}>
              <Col span={8}>
                <Card size="small">
                  <Statistic title={t('battery.charges', '累计充电次数')} value={detailItem.charges} />
                </Card>
              </Col>
              <Col span={8}>
                <Card size="small">
                  <Statistic title={t('battery.status', '充电状态')} value={{
                    idle: '未充电', preparing: '准备充电', charging: '充电中',
                    discharging: '对方放电', fault: '故障&异常'
                  }[detailItem.status] || detailItem.status} />
                </Card>
              </Col>
              <Col span={8}>
                <Card size="small">
                  <Statistic title={t('battery.daily_consumption', '日均电耗')} value={detailItem.dailyConsumption || 0} suffix="kWh/100km" />
                </Card>
              </Col>
            </Row>
            <Card size="small" title={t('battery.daily_consumption', '日均电耗趋势')}>
  ...
  const chargeTab = (
    <div>
      ...
      <Table
        dataSource={filteredChargeRecords}
  ...
  const dischargeTab = (
    <div>
      ...
      <Table
        dataSource={filteredDischargeRecords}
