...
const MileageChart: React.FC = () => {
  ...
  const chartDataMap: Record<string, { labels: string[]; data: number[] }> = {
    day: {
      labels: Array.from({ length: 30 }, (_, i) => `${i + 1}${t('mileage.label.day', '日')}`),
      data: genData(30, 120, 80),
    },
    week: {
      labels: Array.from({ length: 30 }, (_, i) => `${t('mileage.label.week_prefix', '第')}${i + 1}${t('mileage.label.week_suffix', '周')}`),
      data: genData(30, 800, 300),
    },
    month: {
      labels: Array.from({ length: 12 }, (_, i) => `${i + 1}${t('mileage.label.month', '月')}`),
      data: genData(12, 3500, 1200),
    },
    year: {
      labels: Array.from({ length: 3 }, (_, i) => `${2024 + i}${t('mileage.label.year', '年')}`),
  ...
  const chartData = {
    ...
    datasets: [
      {
        label: t('menu.mileage_report', '里程报表') + ' (km)',
    ...
    <div>
      <div style={{ marginBottom: 16, display: 'flex', justifyContent: 'flex-end' }}>
        <Radio.Group value={period} onChange={e => setPeriod(e.target.value)} size="small">
          <Radio.Button value="day">{t('mileage.day', '日')}</Radio.Button>
          <Radio.Button value="week">{t('mileage.week', '周')}</Radio.Button>
          <Radio.Button value="month">{t('mileage.month', '月')}</Radio.Button>
          <Radio.Button value="year">{t('mileage.year', '年')}</Radio.Button>
        </Radio.Group>
      </div>
