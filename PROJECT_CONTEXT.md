...
> 基于 PRD V6（2026-06-17）整理，供 AI 编码助手、新成员快速了解项目全貌。
...
│   │   ├── Repair.tsx          # 维修管理（无编辑，描述联动9+14种，删除回退报警状态）
│   │   ├── Tenant.tsx          # 租户管理
│   │   ├── Biz.tsx             # 业务管理（租户权限/信息/资产多选划拨/用户/角色）
│   │   ├── VehicleSignal.tsx   # 车辆信号数据
│   │   ├── DataExport.tsx      # 数据导出记录
│   │   ├── Sys.tsx             # 系统管理 - 用户管理
│   │   ├── SysRoles.tsx        # 系统管理 - 角色管理
│   │   └── AuditLog.tsx        # 系统管理 - 日志审计（47项操作映射）
│   ├── store/
│   │   └── useAppStore.ts      # Zustand 全局状态（含 localStorage 持久化）
│   ├── types/
...
2. **状态管理**：单一 Zustand Store（`useAppStore`），通过 `persist` 中间件将 `page/user/token/lang/tenant` 持久化到 localStorage
...
| 9 | 维修管理 | 维修记录(无编辑,描述联动,删除回退) | 定制 | 车队录入 |
| 10 | 租户管理 | 租户管理 | 基线 | 车队录入 |
| 11 | 业务管理 | 租户权限 / 租户信息 / 资产划拨(多选) / 用户管理 / 角色管理 | 基线 | 车队录入+IOT |
| 12 | 车辆数据 | 车辆信号数据 / 数据导出记录 | 定制 | OBD |
...
- 新建维修描述支持**类型联动**：`故障类` → 9种故障选项，`电池类` → 14种电池选项
